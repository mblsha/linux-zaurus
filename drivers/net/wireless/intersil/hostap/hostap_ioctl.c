					  ...
					  union iwreq_data *uwrq, char *extra)
{
	...
	case PRISM2_PARAM_AP_SCAN:
		...
		if (timer_pending(&local->passive_scan_timer))
			timer_delete(&local->passive_scan_timer);
...
static const iw_handler prism2_handler[] =
{
	...
	IW_HANDLER(SIOCSIWSPY,		hostap_wx_set_spy),
	IW_HANDLER(SIOCGIWSPY,		hostap_wx_get_spy),
	IW_HANDLER(SIOCSIWTHRSPY,	hostap_wx_set_thrspy),
	IW_HANDLER(SIOCGIWTHRSPY,	hostap_wx_get_thrspy),
